...
## Phase 4: Smoke Ring Rendering

When smoke is ready and the visible face reports `mouthShape: "O_SHAPE"`, the interaction engine emits one typed `SMOKE_RING` command before checking the legacy open-mouth path. The command uses the same mouth origin, yaw-derived direction, and inhale-derived strength as the existing smoke emissions. It consumes pending smoke immediately, so an O-shaped mouth cannot produce repeated rings without another qualifying inhale. If the O-shape condition is absent, `MOUTH_BURST` and the delayed `NOSE_BURST` paths remain unchanged and retain their priority relative to one another.

The ring is emitter kind `3` inside the existing 8,000-point `BufferGeometry`, not a separate system or static geometry. Particles are distributed at stable angles around the mapped mouth origin with small radius/angle/seed/lifetime variation. Their initial velocity combines yaw-directed travel, radial expansion, slight upward buoyancy, and per-particle variation. High/medium/low quality uses `max(180, round((260 + strength × 140) × qualityFactor))` particles, yielding approximately 180–449 particles without changing adaptive thresholds or existing emitter counts.

The shared GLSL path gates ring behavior explicitly by kind ≥3. Ring particles receive late-life analytic wobble and buoyancy, modest point-size growth, a noise-derived breakup mask that introduces gaps after roughly 30% lifetime, and a late fade from roughly 62–100% lifetime. Existing tip, mouth, and nose particles use the previous shader equations and alpha path because their ring gate is zero. Debug mode reports `LAST EFFECT` and `RING COUNT`. No hand interaction, smoke-ring-specific future effects, or additional MediaPipe work is included; the ring's main limitations are heuristic screen-space depth/direction and shared-pool overwrite under heavy concurrent smoke.
